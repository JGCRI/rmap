...
# Create data table with a mis-spelled subRegion
data = data.frame(subRegion = c("Italy","Spain","Greice"),
                  value = c(10,15,34))

# Plot data
rmap::map(data,labels = T, title = "Mis-spelled subRegion 'Greice'")
...
# Fix the subRegion that was mis-spelled
...
```{r, results = 'hide', eval=T, echo=F, warning=FALSE, error = FALSE, message = FALSE, fig.dim=c(15,5)}
library(rmap);
data = example_gridData_GWPv4To2015;
mapx <- rmap::map(data = data,
          overLayer = rmap::mapCountries,
...
```{r, results = 'hide', eval=T, echo=TRUE, warning=FALSE, error = FALSE, message = FALSE, fig.dim=c(15,5)}
library(rmap);library(dplyr)
...
```{r, results = 'hide', eval=F, echo=F, warning=FALSE, error = FALSE, message = FALSE, fig.dim=c(15,5)}
library(rmap);

...
<!-------------------------->
# Key Links
<p align="center"> <img src="vignetteFigs/rmapHeaderThick.PNG"></p>
<!-------------------------->
<!-------------------------->
...
rmap comes with a set of preloaded maps. A full list of rmap maps is available at [colors, maps and params](https://jgcri.github.io/rmap/articles/vignette_colorsMapsParams/#maps-1.html). The pre-loaded maps all come with each polygon labelled in a **subRegion** column. For each map the data contained in the shapefile and the map itself can be viewed as follows:
...
`map` will search through the list of [pre-loaded rmap maps](https://jgcri.github.io/rmap/articles/vignette_colorsMapsParams.html) to see if it can find the "subRegions" provided in the data and then plot the data on those maps. Some examples are provided below:
...
Sometimes subRegions can be present on multiple maps. For example "Colombia", China" and "India" are all members of `rmap::mapGCAMReg32` as well as `rmap::mapCountries`. If a user knows which map they want to plot their data on they should specify the map in the `subRegShape` argument.
...
data = data.frame(subRegion=c("AK","HI","PR","MO","TX","WY"),
                  ...
                  value=c(5,10,15,34,2,7))
map(polygonTable=data,
                  subRegShape=rmap::mapUS52Compact,
                  folderName = "vignetteMaps", mapTitleOn = F)
```
...
data = data.frame(subRegion=c("Aleutians West_AK","Sabana Grande_PR","Kalawao_HI","Orange_IN","Putnam_FL","Ellis_KS"),
                  ...
                  value=c(5,10,15,34,2,7))
map(polygonTable=data,
                  subRegShape=rmap::mapUS52CountyCompact,
                  folderName = "vignetteMaps",
                  nameAppend = "_Alt",
                  mapTitleOn = F)
```
...
mapplot(shapeSubset,fillCol="states") # View custom shape
...
map(polygonTable = data, mapTitleOn = F, folderName = "vignetteMaps", cropToBoundary=F, )
map(polygonTable = data, mapTitleOn = F, folderName = "vignetteMaps", cropToBoundary=T, 
                  nameAppend="Cropped")
```
...
map(polygonTable = data, mapTitleOn=F, folderName = "vignetteMaps", 
                  cropToBoundary =T,
...
data = data.frame(subRegion = c("Austria","Spain", "Italy", "Germany","Greece",
                  ...
                  value = c(32, 38, 54, 63, 24,
                            ...
                            23, 99, 102, 85, 75,
                            12, 76, 150, 64, 90))
map(polygonTable = data, folderName ="multiYear", 
...
data = data.frame(subRegion = c("Austria","Spain", "Italy", "Germany","Greece",
                  ...
                  value = c(32, 38, 54, 63, 24,
                            ...
                            23, 99, 102, 85, 75,
                            12, 76, 150, 64, 90))
map(polygonTable = data, folderName ="multiClass", 
                  cropToBoundary=T, extension = T )
```
...
data = data.frame(subRegion = c("Austria","Spain", "Italy", "Germany","Greece",
                  ...
                  value = c(32, 38, 54, 63, 24,
                            37, 53, 23, 12, 45,
                            40, 44, 12, 30, 99))
map(polygonTable = data, folderName ="multiScenario", 
...
data = data.frame(subRegion = c("Austria","Spain", "Italy", "Germany","Greece",
                  ...
                  value = c(32, 38, 54, 63, 24,
                            37, 53, 23, 12, 45))
map(polygonTable = data, folderName ="scaleRange", 
...
Can use any [R color palette](https://www.nceas.ucsb.edu/sites/default/files/2020-04/colorPaletteCheatsheet.pdf) or choose from a list of rmap color palettes at [colors, maps and params](https://jgcri.github.io/rmap/articles/vignette_colorsMapsParams.html#color-palettes-1).
...
data = data.frame(subRegion = c("Austria","Spain", "Italy", "Germany","Greece",
                  ...
                  value = c(32, 38, 54, 63, 24,
                            37, 53, 23, 12, 45))
map(polygonTable = data, folderName ="colorPalettes", 
                  cropToBoundary=T, extension = T, scenRef="scen1",
...
In this example the preloaded GCAM output `rmap::exampleGCAMproj` is used to extract data for chosen parameters from the [parameter list](https://jgcri.github.io/rmap/articles/vignette_colorsMapsParams.html#params-1). USers can provided a path to their gcam database folder.
...
map(polygonTable = dfParam, folderName ="GCAMbyParam", 
...
map(polygonTable = dfClass1, folderName ="GCAMbyClass", 
                  cropToBoundary=T, extension = T, xRange = c(2010,2020,2030,2040),
                  scenRef="SSP3")
```

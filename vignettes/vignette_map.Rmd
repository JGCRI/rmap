...
map(data = data, mapTitleOn = F, folder = "vignetteMaps", cropToBoundary=F, )
map(data = data, mapTitleOn = F, folder = "vignetteMaps", cropToBoundary=T, 
                  nameAppend="Cropped")
```
...
library(rmap)
data = data.frame(subRegion = c("India","China"), year = c(2010,2010), value = c(32,54))
map(data = data, mapTitleOn = F, folder = "vignetteMaps", 
                  cropToBoundary = T,
...
<p align="center" style="font-size:18px;"> *Legend Position - Multi Facet Maps* </p>
...
* Number of legend breaks can be set using the ```legendFixedBreaks``` variable.
* "Pretty" legends will choose the number of breaks closest to the value of ```legendFixedBreaks``` while ensuring "pretty" number distributions.
* "Freescale" legends will set the number of breaks to exactly the value chosen for ```legendFixedBreaks```
* "Kmeans" will ignore the value chosen for ```legendFixedBreaks```since it calculates the best breaks it self to highlight the distribution of the data.
* The ```legendFixedBreaks``` will apply in the same way to Diff Plots.
...
<p align="center" style="font-size:18px;"> *Legend Position - Legend Breaks* </p>
<p align="center"> <img src="vignetteFigs/map_legendBreaks.PNG"></p>
...
map(data = dfClass1, folder = "GCAMbyClass", 
                  cropToBoundary = T, background = T, xRange = c(2010,2020,2030,2040),
                  scenRef = "SSP3")
```

...
#' @param palx Palette name to view the palette colors. Eg. colors("pal_Basic")
...
colors <- function(palx=NULL) {
  ...
  add.alpha <- function(col, alpha=1){
    ...
    if(length(names(pal_rmap)[duplicated(names(pal_rmap))])>0){
      stop(paste("Duplicated color names in pal_rmap: ",paste(names(pal_rmap)[duplicated(names(pal_rmap))],collapse=", "),sep=""))
    }
